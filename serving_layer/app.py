...
def get_system_insights():
    """
    Return pre-computed system-wide AI insight summary.
    Generated by ai_batch_processor.py and stored in system_insights collection.
    """
    db, client = _get_ai_db()
    try:
        doc = db["system_insights"].find_one({"type": "latest"}, {"_id": 0})
        if not doc:
            return jsonify({
                "status":  "ok",
                "message": "No AI insights yet. Run batch_layer/ai_batch_processor.py first.",
                "data":    None,
            })
        doc.pop("type", None)
...
# ── /api/ai/insight/<city> ─────────────────────────────────────────────────────
@app.route("/api/ai/insight/<city_name>")
def get_city_insight(city_name):
    """
    Return AI-generated insight for one city.
    Includes headline, summary, key insights, alerts, actions, outlook.
    Falls back to live generation if pre-computed not available.
    """
    db, client = _get_ai_db()
    try:
        lookup = CITY_ALIASES.get(city_name, city_name)
        ...
        # Return pre-computed insight if available
        pre = batch.get("city_insight")
        if pre:
            return jsonify({"status": "ok", "city": lookup, "data": pre, "source": "precomputed"})

        # Live fallback generation
        try:
            from ai_layer.insight_generator import InsightGenerator
            rt      = db["realtime_views"].find_one({"city": lookup}, {"_id": 0}) or {}
            trend   = db["trend_profiles"].find_one({"city": lookup}, {"_id": 0}) or {}
            ...
            return jsonify({"status": "ok", "city": lookup, "data": insight, "source": "live"})
        except ImportError:
            return jsonify({
                "status":  "ok",
                "message": "AI layer not available. Run ai_batch_processor.py to pre-compute.",
                "data":    None,
            })
    finally:
        client.close()
...
# ── /api/ai/forecast/<city> ────────────────────────────────────────────────────
@app.route("/api/ai/forecast/<city_name>")
def get_ai_forecast(city_name):
    """
    Return ML-based multi-horizon forecast for a city.
    Horizons: 6h, 12h, 24h, 7d — each with confidence interval + model info.
    """
    db, client = _get_ai_db()
    try:
        lookup = CITY_ALIASES.get(city_name, city_name)
        ...
        try:
            ...
            if horizon == "all":
                forecasts = engine.predict_all_horizons(
                    city=lookup,
                    current_reading=rt,
                    batch_stats=stats,
                )
                comparison = engine.get_model_comparison(lookup, "24h")
                return jsonify({
                    "status":     "ok",
                    "city":       lookup,
                    "forecasts":  forecasts,
                ...
                forecast   = engine.predict(lookup, horizon, rt, stats)
                comparison = engine.get_model_comparison(lookup, horizon)
                return jsonify({
                    "status":    "ok",
                    "city":      lookup,
                    "horizon":   horizon,
            ...
            # Fallback: use existing monthly_trend linear extrapolation
            monthly = sorted(batch.get("monthly_trend", []), key=lambda x: x["month"])
            avg_usi = stats.get("avg_usi", 40) or 40
            std_usi = stats.get("stddev_usi", 5) or 5
            MONTHS  = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]
            ...
            for i in range(12):
                month_idx = (start + i - 1) % 12 + 1
                seasonal  = next((m["avg_usi"] for m in monthly if m["month"] == month_idx), avg_usi)
                predicted = round(seasonal * 0.7 + avg_usi * 0.3, 2)
                forecasts_list.append({
                    "month":         MONTHS[month_idx - 1],
                    "predicted_usi": predicted,
                    "upper_bound":   round(predicted + std_usi * 0.8, 2),
                    "lower_bound":   round(max(0, predicted - std_usi * 0.8), 2),
                })
            return jsonify({
                "status":   "ok",
                "city":     lookup,
                "horizon":  horizon,
                "forecast": {"predictions": forecasts_list, "source": "fallback_linear"},
                "model_comparison": [],
            })
    finally:
        client.close()
...
# ── /api/ai/anomaly/<city> ─────────────────────────────────────────────────────
@app.route("/api/ai/anomaly/<city_name>")
def get_anomaly_detail(city_name):
    """
    Return detailed anomaly analysis for a city's current reading.
    Includes ensemble scores, severity, feature z-scores, and explanation.
    """
    db, client = _get_ai_db()
    try:
        lookup = CITY_ALIASES.get(city_name, city_name)
        ...
        aqi   = rt.get("aqi",         100) or 100
        temp  = rt.get("temperature",  26)  or 26
        hum   = rt.get("humidity",     50)  or 50
        usi   = rt.get("usi",          40)  or 40
        ...
        try:
            ...
            if ensemble:
                ...
                results = ensemble.predict_batch(X)
                result  = results[0]
            else:
                result = {
                    "is_anomaly":       rt.get("is_anomaly", aqi > 200),
                    "anomaly_score":    min(aqi / 300, 1.0),
                    "confidence_pct":   40,
                    "severity":         "High" if aqi > 200 else "Normal",
                    "models_flagged":   ["threshold"],
                    "method":           "threshold",
                    "feature_zscores":  {},
                }
            ...
            explanation = explainer.explain_anomaly(
                city=lookup,
                aqi=aqi, temperature=temp, humidity=hum, usi=usi,
                feature_zscores=result.get("feature_zscores", {}),
                severity=result.get("severity", "Normal"),
                models_flagged=result.get("models_flagged", []),
            ...
            return jsonify({
                ...
                "current_reading": {
                    "aqi": aqi, "temperature": temp,
                    "humidity": hum, "usi": usi,
            ...
            return jsonify({
                ...
                "detection": {
                    ...
                    "anomaly_score": min(aqi / 300, 1.0),
                    "severity":      rt.get("risk_level", "Normal"),
                    "method":        "threshold",
                },
                "explanation": None,
                "message":     "Full AI ensemble not available — run ai_batch_processor.py",
            })
    finally:
        client.close()
...
# ── /api/ai/trends/<city> ──────────────────────────────────────────────────────
@app.route("/api/ai/trends/<city_name>")
def get_ai_trends(city_name):
    """
    Return full trend intelligence profile for a city.
    Includes monthly/seasonal/yearly patterns, volatility, pollutant ranking.
    """
    db, client = _get_ai_db()
    try:
        lookup = CITY_ALIASES.get(city_name, city_name)
        trend  = db["trend_profiles"].find_one({"city": lookup}, {"_id": 0})

        if not trend:
            # Fall back to batch_views monthly/yearly data
            batch = db["batch_views"].find_one({"city": lookup}, {"_id": 0})
            if not batch:
                return jsonify({"status": "error", "message": f"City '{city_name}' not found"}), 404
            return jsonify({
                "status":  "ok",
                "city":    lookup,
                "source":  "batch_views_fallback",
                "data": {
                    "monthly_trend": batch.get("monthly_trend", []),
                    "yearly_trend":  batch.get("yearly_trend",  []),
                    "message":       "Full trend profile not available. Run ai_batch_processor.py.",
                },
            })

        trend.pop("_id", None)
        return jsonify({"status": "ok", "city": lookup, "source": "trend_profiles", "data": trend})
    finally:
        client.close()


# ── /api/ai/correlation/<city> ─────────────────────────────────────────────────
@app.route("/api/ai/correlation/<city_name>")
def get_ai_correlation(city_name):
    """
    Return enhanced correlation data including pollutant ranking.
    Combines Pearson matrix + TrendIntelligence pollutant ranking.
    """
    db, client = _get_ai_db()
    try:
        lookup = CITY_ALIASES.get(city_name, city_name)

        # Pearson matrix from correlations collection
        corr_doc = db["correlations"].find_one({"city": lookup}, {"_id": 0})

        # Pollutant ranking from trend_profiles
        trend    = db["trend_profiles"].find_one({"city": lookup}, {"_id": 0}) or {}
        poll_rank = trend.get("pollutant_ranking", [])

        if not corr_doc:
            return jsonify({"status": "error", "message": f"No correlation data for '{city_name}'"}), 404

        corr = corr_doc.get("correlations", {})
        matrix = [
            [1.0,                           corr.get("aqi_vs_temp",  0), corr.get("aqi_vs_hum",  0), corr.get("aqi_vs_usi",  0)],
            [corr.get("aqi_vs_temp",  0),   1.0,                          corr.get("temp_vs_hum", 0), corr.get("temp_vs_usi", 0)],
            [corr.get("aqi_vs_hum",  0),    corr.get("temp_vs_hum", 0),  1.0,                         corr.get("hum_vs_usi",  0)],
        ...
        return jsonify({
            ...
            "correlation": {
                "matrix":             matrix,
                "labels":             ["AQI", "temperature", "humidity", "usi"],
                "sample_size":        corr_doc.get("sample_size", 0),
                "raw":                corr,
                "pm25_vs_aqi":        corr.get("pm25_vs_aqi"),
                "pm10_vs_aqi":        corr.get("pm10_vs_aqi"),
                "no2_vs_aqi":         corr.get("no2_vs_aqi"),
            },
            "pollutant_ranking": poll_rank,
        })
    finally:
        client.close()
...
def get_model_registry():
    """
    Return model registry summary — all trained models, metrics, training dates.
    """
    db, client = _get_ai_db()
    try:
        models = list(db["ai_models"].find({}, {"_id": 0, "model_b64": 0}))
        summary = {
            "total_models":       len(models),
            "cities_with_models": len(set(m.get("city") for m in models)),
            "model_types":        list(set(m.get("model_type") for m in models)),
            "best_models":        sum(1 for m in models if m.get("is_best")),
        }
        return jsonify({"status": "ok", "summary": summary, "models": models})
    finally:
        client.close()


# ── /api/ai/explain/<city> ─────────────────────────────────────────────────────
@app.route("/api/ai/explain/<city_name>")
def get_risk_explanation(city_name):
    """
    Return human-readable explanation of a city's current risk score.
    Breaks down USI into components and generates actionable text.
    """
    db, client = _get_ai_db()
    try:
        lookup = CITY_ALIASES.get(city_name, city_name)
        ...
        stats      = batch.get("stats", {})
        aqi        = (rt or {}).get("aqi",         stats.get("avg_aqi", 100)) or 100
        temp       = (rt or {}).get("temperature", stats.get("avg_temp", 26))  or 26
        hum        = (rt or {}).get("humidity",    stats.get("avg_humidity",50))or 50
        usi        = (rt or {}).get("usi",         stats.get("avg_usi", 40))   or 40
        risk_level = (rt or {}).get("risk_level",  "Moderate") or "Moderate"

        try:
            from ai_layer.explainability import Explainability
            exp = Explainability()
            explanation = exp.explain_city_risk(
                city=lookup,
                usi=usi, aqi=aqi,
            ...
            return jsonify({"status": "ok", "city": lookup, "explanation": explanation})
        except ImportError:
            return jsonify({
                "status": "ok",
                "city":   lookup,
                "explanation": {
                    "summary":    f"{lookup} USI={usi:.1f} ({risk_level} risk)",
                    "components": [],
                    "message":    "Full AI explainability not available.",
                },
            })
    finally:
        client.close()